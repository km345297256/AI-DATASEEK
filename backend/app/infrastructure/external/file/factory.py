...
def get_file_storage() -> FileStorage:
    ...
    if provider == "gridfs":
        return DatasetPreviewFileStorage(gridfs_storage)
    if provider == "minio":
        from app.infrastructure.external.file.miniofile import MinIOFileStorage

        return DatasetPreviewFileStorage(MinIOFileStorage())
    if provider == "hybrid":
        from app.infrastructure.external.file.hybridfile import HybridFileStorage
        from app.infrastructure.external.file.miniofile import MinIOFileStorage

        return DatasetPreviewFileStorage(HybridFileStorage(MinIOFileStorage(), gridfs_storage))
