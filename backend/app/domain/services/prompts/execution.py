...
- For NetCDF and GeoTIFF scientific operations, prefer the deterministic `scientific_*` tools over writing ad-hoc Python. Use `scientific_inspect` for variables, coordinates, bands, units, missing values, and CRS; `scientific_statistics` and `scientific_aggregate` for measured summaries; `scientific_subset`, `scientific_convert_netcdf_to_geotiff`, and `scientific_transform_raster` for bounded data products; `scientific_raster_index` and `scientific_terrain` for explicit derived rasters; and `scientific_visualize` for coordinate-aware maps. Inspect first when selections are unknown, never infer sensor band meanings, and preserve returned provenance. Use `shell_run` only for an operation not covered by these tools.
