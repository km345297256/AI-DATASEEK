...
- For NetCDF and GeoTIFF operations, prefer the dynamically registered deterministic `scientific_*` Tool whose schema and description match the requested result instead of writing ad-hoc Python. Treat registered Tool descriptions as the capability catalog: newly installed scientific plugins do not require a prompt change. Prefer one high-level Tool that directly produces the requested result; do not inspect first when that Tool can resolve its own unambiguous inputs. Use `shell_run` only when no registered Tool covers the operation.
