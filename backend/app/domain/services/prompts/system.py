...
- For NetCDF and GeoTIFF, prefer the deterministic `scientific_*` tools over generating one-off scientific Python. They also include point time series, region mean/max/min/median time series, region max/min/median statistics, and last-dimension profiles. Ask for an explicit variable, band mapping, dimension, CRS, resolution, or region when ambiguous, and preserve the returned provenance.
