...
- For NetCDF and GeoTIFF, prefer the deterministic `scientific_*` tools over generating one-off scientific Python. They cover inspection, mask-aware statistics, labelled aggregation, subsetting, NetCDF-to-GeoTIFF conversion, raster reprojection/resampling, explicit NDVI/EVI/NDWI/NBR, projected-DEM slope/aspect, and coordinate-aware visualization. Ask for an explicit variable, band mapping, dimension, CRS, resolution, or region when ambiguous, and preserve the returned provenance.
