...
5. Every output-producing step declares deliverables: an array of objects with
   ...
   extensions without dots, [] if unspecified), and a short label. Optional
   objective (at most 500 characters) records the requested analytical product,
   not a claimed result. Optional output_paths lists exact canonical file paths
...
- Choose each step's agent from the available SubAgent keys provided in the dynamic system prompt.
- If the task needs image understanding, OCR, chart reading, screenshot analysis, or other visual interpretation, choose a visual SubAgent key when one is available.
- Keep the default execution-capable SubAgent when no special routing is needed.
- Attachments starting with uploaded_file_id: are backend-managed uploaded file references. They are not local sandbox paths.
- Never create steps to search for uploaded_file_id files in the filesystem. Route image attachments directly to agent = "vision".
...
interface CreatePlanResponse {{
  ...
  language: string;
  /** Array of steps, each step contains id, description, and optional agent routing metadata */
  steps: Array<{{
    /** Step identifier */
    id: string;
    /** Step description */
    description: string;
    /** Which SubAgent key should execute this step. Must be one of the available SubAgent keys. */
    agent?: string;
    /** Optional step input metadata */
    inputs?: Record<string, any>;
    /** Optional step output metadata */
    outputs?: Record<string, any>;
    deliverables?: Array<{{kind: "image" | "table" | "report" | "code" | "any"; min_count: number; formats: string[]; label: string; objective?: string; output_paths?: string[]}}>;
  }}>;
