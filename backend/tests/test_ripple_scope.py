"""Ripple two-file capability security; isolated fake dataset storage only."""
import asyncio
from types import SimpleNamespace
import pytest
from app.application.services import ripple_scope as module
...
def configure(env, host=False, header="folder/cube.rpl", data="folder/cube.raw"):
    ...
    objects={header:b"Ripple synthetic header",data:b"binary-samples", "other/cube.raw":b"secret", "folder/private.txt":b"secret"}
...
async def scope_for(env, **kwargs):
    ...
    info=(await prepare(env,path=kwargs.get("header","folder/cube.rpl"),plugin=PLUGIN)).file
    env.state["reads"].clear()
    scope=await module.RippleDatasetScope(env.service,info.file_id,"owner").initialize()
    return scope
...
async def test_exact_pair_stat_only_no_names_or_paths_in_public_state(env,host):
    ...
    assert {r[1] for r in env.state["reads"]} == {"folder/cube.rpl","folder/cube.raw"}
    public=scope.info.model_dump_json()
    for secret in ("private-source","/allowed","folder/","sources/","cube.raw","private.txt"):
        assert secret not in public
    assert scope.info.file_path is None and scope.info.file_url is None

@pytest.mark.asyncio
@pytest.mark.parametrize("header,data",[("cube.rpl","cube.raw"),("folder/cube.rpl","folder/cube.raw"),("folder/cube.RPL","folder/cube.RAW")])
...
async def test_ambiguous_or_missing_pair_fails_before_any_stat(env):
    configure(env); info=(await prepare(env,path="folder/cube.rpl",plugin=PLUGIN)).file
    for change in ("extra","missing"):
        env.state["reads"].clear()
        if change=="extra": env.datasets.dataset.files.append(DatasetFile(path="folder/cube.RAW",size=1))
        else: env.datasets.dataset.files=[f for f in env.datasets.dataset.files if f.path not in {"folder/cube.raw","folder/cube.RAW"}]
        with pytest.raises(ScientificPreviewRejected): await module.RippleDatasetScope(env.service,info.file_id,"owner").initialize()
...
async def test_range_cannot_cross_members_or_read_undeclared_siblings(env):
    ...
    assert len(env.state["reads"])==1 and env.state["reads"][0][1]=="folder/cube.raw"
...
@pytest.mark.asyncio
@pytest.mark.parametrize("member",["folder/cube.rpl","folder/cube.raw"])
async def test_version_includes_both_members_even_if_selected_window_did_not_read_it(env,member):
    ...
    newer=await module.RippleDatasetScope(env.service,scope.source_id,"owner").initialize()
...
async def test_plugin_off_and_unapproved_parameters_never_establish_scope(env):
    ...
    with pytest.raises(VisualizationDisabledError): await module.ripple_visualization(storage,env.catalog,"image",info.file_id,"owner",VisualizationRequest(plugin_id=PLUGIN,operation="preview"))
    ...
    for request in (VisualizationRequest(plugin_id=PLUGIN,operation="preview",kind="tree",options={"resource_id":"foreign"}),
                    VisualizationRequest(plugin_id=PLUGIN,operation="preview",kind="image",options={"channel":0,"x":0,"y":0,"width":1,"height":1})):
        with pytest.raises(ScientificPreviewRejected): await module.ripple_visualization(storage,env.catalog,"image",info.file_id,"owner",request)
    assert not env.state["reads"]
...
async def test_standalone_uploaded_header_does_not_acquire_any_related_file(env):
    ...
    with pytest.raises(ScientificPreviewRejected,match="数据集"):
        await module.ripple_visualization(SimpleNamespace(),env.catalog,"image","opaque-upload","owner",VisualizationRequest(plugin_id=PLUGIN,operation="preview"))
    assert not env.state["reads"]

@pytest.mark.asyncio
async def test_mid_read_member_change_is_rejected(env):
    scope=await scope_for(env)
    env.state["on_data"] = lambda: env.state["versions"].__setitem__("folder/cube.rpl",2)
...
async def test_scope_final_fence_is_passed_to_existing_window_service(env,monkeypatch):
    ...
    async def window(scope,catalog,image,file_id,owner,request,**kwargs):
        ...
        env.state["versions"]["folder/cube.raw"]+=1
        await kwargs["final_fence"]()
    monkeypatch.setattr(module,"window_visualization",window)
    with pytest.raises(PreviewVersionChanged):
        await module.ripple_visualization(SimpleNamespace(_file_storage=SimpleNamespace(previews=env.service)),env.catalog,"image",info.file_id,"owner",VisualizationRequest(plugin_id=PLUGIN,operation="preview"))
