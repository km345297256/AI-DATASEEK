...
> 人工数据集设置页面已移除。第三方系统直接调用本文档中的 API，然后自行把浏览器重定向到探查页面。

> `7000` 是 AI-DataSeek 唯一保留的服务端口。实际接入时可将示例地址统一替换为正式部署域名或地址。
...
提交接口只返回 JSON，**不会由 AI-DataSeek API 直接返回 302**。302 响应应由第三方 Web 服务生成。
...
> 当前接口只接收 JSON，不接收 `application/x-www-form-urlencoded` 或 `multipart/form-data`。
