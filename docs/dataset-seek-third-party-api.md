...
提交内容不会写入数据集数据库。真实服务器目录不会写入 URL、`localStorage` 或 `sessionStorage`；跳转 URL 中只包含临时 `dataset_id`，页面可能使用该 ID 在 `localStorage` 中关联当前分析会话。

- 临时数据集默认有效期：24 小时（当前版本不可通过该提交接口调整）；
- 共享系统管理员身份最多保留 16 个临时数据集，超过后淘汰最早的一项；
- 系统整体最多保留 128 个临时数据集；
- 后端服务重启后，所有临时 `dataset_id` 立即失效；
- 临时注册表位于单个后端进程内；如自行部署多个后端进程或副本，必须保证提交和后续页面/API 请求命中同一进程，否则会被视为不存在；
- 相同参数可以重复提交，每次都会返回新的 `dataset_id`；
- `external_id` 仅作为第三方业务标识回显，不做唯一性校验或幂等控制。

第三方系统应在提交成功后立即跳转，不应长期保存临时 `dataset_id`。
...
成功时的响应包装和 `data` 结构与提交接口相同；不存在、已过期、被淘汰或属于其他后端进程时返回 `404`。
...
| `404` | 临时数据集已过期、服务已重启或被容量策略淘汰 | 重新提交后再访问 |
