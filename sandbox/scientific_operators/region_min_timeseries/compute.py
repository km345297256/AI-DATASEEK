...
对每个时间步，在多边形/bbox/全场有效像元上做空间 ``nanmin``，得到 1D 时间序列。
对齐 Portal「{变量} · 选区最小时间序列」与 ``tpdc_batch.region_timeseries(..., reduce="min")``。
"""
...
# ---------------------------------------------------------------------------
# Core reduce（对齐 _reduce_timeseries_cube(..., reduce="min")）
# ---------------------------------------------------------------------------


def spatial_min_series(
    cube: np.ndarray,
    mask_outside: np.ndarray | None = None,
) -> np.ndarray:
    """
    ``cube``: (time, lat, lon) → (time,) 每步空间最小值。
...
def chart_title(*, variable: str = "", mode: str = "polygon") -> str:
    """对齐 RegionTimeChart：``snow · 选区最小时间序列``。"""
    v = variable or "变量"
    scope = "全场" if mode == "full" else "选区"
    return f"{v} · {scope}最小时间序列"
...
def min_timeseries(
    ...
    *,
    ...
    Returns:
        与 ``POST .../region-timeseries``（reduce=min）核心字段对齐的 dict：
    ...
    series = spatial_min_series(arr, mask_outside)
