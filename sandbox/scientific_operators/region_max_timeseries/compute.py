...
对每个时间步，在多边形/bbox/全场有效像元上做空间 ``nanmax``，得到 1D 时间序列。
对齐 Portal「{变量} · 选区最大时间序列」与 ``tpdc_batch.region_timeseries(..., reduce="max")``。
"""
...
# ---------------------------------------------------------------------------
# Core reduce（对齐 _reduce_timeseries_cube(..., reduce="max")）
# ---------------------------------------------------------------------------


def spatial_max_series(
    cube: np.ndarray,
    mask_outside: np.ndarray | None = None,
) -> np.ndarray:
    """
    ``cube``: (time, lat, lon) → (time,) 每步空间最大值。
...
def chart_title(*, variable: str = "", mode: str = "polygon") -> str:
    """对齐 RegionTimeChart：``snow · 选区最大时间序列``。"""
    v = variable or "变量"
    scope = "全场" if mode == "full" else "选区"
    return f"{v} · {scope}最大时间序列"
...
def max_timeseries(
    ...
    *,
    ...
    Returns:
        与 ``POST .../region-timeseries``（reduce=max）核心字段对齐的 dict：
    ...
    series = spatial_max_series(arr, mask_outside)
