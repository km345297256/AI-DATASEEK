...
对每个时间步，在多边形/bbox/全场有效像元上做空间 ``nanmean``，得到 1D 时间序列。
对齐 Portal「区域平均时间序列分析」与 ``recipes.timeseries.apply_recipe(..., "spatial_mean")``。
"""
...
def spatial_mean_series(
    ...
    """
    ``cube``: (time, lat, lon) → (time,) 每步空间平均值。
...
def chart_title(*, variable: str = "", mode: str = "polygon") -> str:
    """对齐 Portal：``{变量} 区域平均时间序列``。"""
    v = variable or "变量"
    scope = "全场" if mode == "full" else "选区"
    return f"{v} · {scope}平均时间序列"
...
def avg_timeseries(
    ...
    *,
    ...
    series = spatial_mean_series(arr, mask_outside)
