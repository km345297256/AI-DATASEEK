"""栅格最大值工具：在全场或选区内求 max，并返回极值经纬度。

逻辑对齐 launch-api `tpdc_batch._field_stats` / `region_layer_stats`
中「最大」「最大值位置」字段（对应 Portal 选区统计 KPI）。
"""
...
def find_max(
    ...
    无效像元（NaN / Inf）忽略。无有效像元时 max / max_location 为 None。

    Returns:
        {
          "max": float | None,
          "max_location": {"lat": float, "lon": float} | None,
          "valid_count": int,
    ...
    amax = float(np.nanmax(flat))
    max_loc = None
    index = None
    try:
        if flat.ndim == 2 and lat1.ndim == 1 and lon1.ndim == 1:
            if lat1.size == flat.shape[0] and lon1.size == flat.shape[1]:
                jj, ii = np.unravel_index(np.nanargmax(flat), flat.shape)
                max_loc = {"lat": float(lat1[jj]), "lon": float(lon1[ii])}
...
def find_max_in_region(
    ...
    *,
    ...
    """
    在多边形或 bbox 选区内求最大值（对应 UI「选区统计 → 最大 / 最大值位置」）。

    Returns（与 region-stats 相关字段对齐，仅保留最大值语义）:
        {
          "mode": "polygon" | "bbox",
    ...
    result = find_max(masked, lat, lon)
...
def find_max_from_npy(
    ...
    *,
    ...
    典型用途：TPDC 任务目录下的 ``max.npy`` / ``lat.npy`` / ``lon.npy``
    （或 min.npy / median.npy 图层上再求空间最大）。
    若提供 polygon/bbox，则只在选区内求最大；否则全场。
    """
...
def format_max_location(loc: dict[str, float] | None, *, digits: int = 2) -> str:
...
def format_max_value(value: float | None, *, precision: int = 2) -> str:
