    ...
    <div class="flex-1 min-w-0 p-4 flex flex-col h-full">
      ...
      <div v-if="toolInfo" class="flex items-center gap-2 mt-2">
        ...
        <div class="flex-1 flex flex-col gap-1 min-w-0">
          <div class="text-[12px] text-[var(--text-tertiary)]">{{ $t('AI-DataSeek is using') }} <span
              class="text-[var(--text-secondary)]">{{ toolInfo.name }}</span></div>
          <div :title="toolInfo.functionArg ? `${toolInfo.function} ${toolInfo.functionArg}` : toolInfo.function"
