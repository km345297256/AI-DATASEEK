<template>
  <section class="mb-6 rounded-xl border border-[var(--border-main)] bg-[var(--background-menu-white)] p-4 sm:p-5">
    ...
    <p class="mb-4 text-xs text-[var(--text-tertiary)]">协议 v1 · 只读文件 · 可信适配器按需加载 · 地图使用离线底图，无外部地图请求<span v-if="catalog"> · Cordis {{ catalog.revision.slice(0, 12) }}</span></p>
    ...
    <div v-if="catalog" class="grid gap-3 lg:grid-cols-2">
      <article v-for="plugin in filteredPlugins" :key="plugin.id" class="min-w-0 rounded-lg border border-[var(--border-main)] bg-[var(--background-gray-main)] p-4" :data-plugin-id="plugin.id">
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0"><h3 class="text-sm font-semibold text-[var(--text-primary)]">{{ plugin.name }}</h3><p class="mt-1 text-[11px] text-[var(--text-tertiary)]">{{ plugin.id }} · v{{ plugin.version }}</p></div>
        ...
        <p class="mt-2 text-[11px] text-[var(--text-tertiary)]">读取上限 {{ byteLabel(plugin.limits.max_input_bytes) }}{{ plugin.reader === 'fastq' ? '（前缀抽样）' : ['text', 'csv'].includes(plugin.adapter) ? '（每页；不限制原文件总大小）' : '' }} · {{ plugin.adapter }}</p>
      </article>
    </div>
