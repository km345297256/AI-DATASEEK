      ...
      <header class="sm:h-auto sticky top-0 left-0 right-0 z-10" style="background: var(--background-share-header);">
        <div
          ...
          <div
            ...
            {{ title }}</div>
          <div class="flex items-center sm:gap-3"><button v-if="!isAdminReplay" @click="handleCopyLink"
        ...
        <div class="flex flex-col w-full gap-[12px] pb-[80px] pt-[12px] flex-1 overflow-y-auto">
          ...
          <div v-if="completionAdvice" class="rounded-xl border border-[var(--border-main)] bg-[var(--background-white-main)] p-4">
            ...
            <div class="flex flex-col gap-2 text-sm text-[var(--text-secondary)]">
              <div v-for="(item, index) in completionAdvice.recommendations" :key="index" class="rounded-lg bg-[var(--background-gray-main)] px-3 py-2">
                {{ item }}
              </div>
            </div>
    ...
    <div v-if="showReplayOverlay"
      ...
      <div class="flex flex-col items-center gap-4 p-2.5">
        ...
        <div class="text-center text-[var(--text-primary)] whitespace-pre-line"
          v-html="$t('You are viewing a completed AI-DataSeek task. Replay will start automatically in {countdown} seconds.', { countdown: `<strong>${countdown}</strong>` })">
        </div>
      </div>
    </div>
...
import { getAdminTaskReplay } from '@/api/admin';
...
const loadReplaySession = async (id: string) => {
  return isAdminReplay.value ? getAdminTaskReplay(id) : agentApi.getSharedSession(id);
};
