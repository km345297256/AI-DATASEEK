...
export const normalizeBackendDateTime = (value: string): string => {
  if (!value) return value;
  return HAS_TIMEZONE_SUFFIX.test(value) ? value : `${value}Z`;
};
...
export const parseISODateTime = (isoString: string): number => {
  try {
    const date = new Date(normalizeBackendDateTime(isoString));
...
export const parseBackendDate = (value: string): Date => {
  ...
  if (isNaN(date.getTime())) {
    throw new Error(`Invalid backend datetime string: ${value}`);
  }
  return date;
};

export const formatDateTime = (value: string, locale?: string): string => {
  return parseBackendDate(value).toLocaleString(locale, {
...
export const formatDate = (value: string, locale?: string): string => {
  return parseBackendDate(value).toLocaleDateString(locale, {
    timeZone: DISPLAY_TIME_ZONE,
    year: 'numeric',
    month: 'short',
    day: 'numeric',
